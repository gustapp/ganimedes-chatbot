...
with click.progressbar(course_id_list) as course_id_bar:
    for course_id in course_id_bar:
        try:
            nome = ""
            creditos_aula = ""
            creditos_trabalho = ""
            ...
            for line in range(len(x)):
                ...
                if(texto == 'Docente(s) Responsável(eis)'):
                    counter = 1
                    while(counter < 20):
                        docente = x[line + counter].strip()
                        if docente == 'Programa Resumido':
                            break
                        docentes_list.append(docente)
                        counter += 1
                    if(counter == 10):
                        log_messages.append('WARNING:max docentes:{}'.format(course_id))
            ...
            try:
                try:
                    driver.find_element_by_xpath('//*[@id="web_mensagem"]/p')
                except:
            ...
            log_messages.append('ERROR:GET:' + course_id)

#%%
import json

with open('db_poli_v2.json','w', encoding='utf8') as outfile:
    json.dump(course_list,outfile,indent=4, ensure_ascii=False)
